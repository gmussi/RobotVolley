...
import { drawHintText, drawPadGlyph } from "./glyphs.js";
...
export function centerText(ctx, txt, color, size, y, shadow = true) {
  ...
  // Routed through the glyph layer so serve/banner prompts can carry [tokens];
  // plain strings take its single-fillText fast path.
  drawHintText(ctx, txt, W / 2, y, size);
  ctx.shadowBlur = 0;
}
...
export function drawFooterHint(ctx, lines, yStart = H - 62) {
  ...
  lines.forEach((line, i) => {
    const size = line.accent ? 14 : 13;
    ctx.font = fontBody(size, line.accent ? 600 : 400);
    ctx.fillStyle = line.accent ? COLORS.accent : COLORS.textMuted;
    drawHintText(ctx, line.text, W / 2, yStart + i * 22, size);
  });
}

/**
 * A key cap. `glyph` is cap text, or `{pad: "cross"}` to draw a controller
 * button in the same frame — used by the controls screen, which shows whichever
 * device the player is currently holding.
 */
export function drawKeyCap(ctx, cx, cy, glyph, accent) {
  const pad = typeof glyph === "object" ? glyph?.pad : null;
  const wide = !pad && glyph.length > 1;
  const w = wide ? 78 : 46;
  const h = 46;
  const x = cx - w / 2;
  const y = cy - h / 2;
